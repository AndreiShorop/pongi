/* autogenerated by Processing revision 1313 on 2026-04-15 */
...
public int[] getAbilityColor(int ab) {
...
float[] BALL_SPEEDS = {5.0f, 7.0f, 9.5f};
...
public int calcXPtoNext(int lvl) {
  return 200 + lvl * 100;
}
...
public void loadProgress() {
  try {
    ...
    if (lines != null && lines.length >= 4) {
      playerXP = PApplet.parseInt(lines[0]);
      playerLevel = PApplet.parseInt(lines[1]);
      currentSkin = constrain(PApplet.parseInt(lines[2]), 0, SKIN_COUNT - 1);
      ballSpeedSetting = constrain(PApplet.parseInt(lines[3]), 0, 2);
      xpToNext = calcXPtoNext(playerLevel);
    }
  } catch (Exception e) {
  }
...
// ===================== XP BAR =====================
public void drawXPBar(float x, float y, float w, float h) {
  noStroke();
  fill(30, 30, 50, 200);
  rect(x, y, w, h, h / 2);
...
// ===================== ABILITY SELECT =====================
public void drawAbilitySelect(int player) {
  int[] bg = skinBG[currentSkin];
  background(bg[0], bg[1], bg[2]);
  if (currentSkin == 0 || currentSkin == 2) drawStarfield();
  if (currentSkin == 1) drawNeonGrid();
...
public void drawField() {
  ...
  if (rallyCount >= fireThreshold) {
    noFill();
    int[] acc = skinAccent[currentSkin];
    stroke(acc[0], acc[1], acc[2], (int)(30 + sin(millis() / 200.0f) * 20));
    strokeWeight(4);
...
public void drawBall() {
  ...
  if (ballOnFire) {
    ...
    ellipse(ballX, ballY, ballSize * 2.5f, ballSize * 2.5f);
    fill(bc[0], bc[1], bc[2]);
  }
...
public void drawTrail() {
  ...
  for (int i = 0; i < trailX.length; i++) {
    ...
    if (ballOnFire) {
      ...
      fill(bc[0], bc[1], bc[2], a * 0.4f);
    }
    float s = map(i, 0, trailX.length, ballSize * 0.9f, 2);
...
public void drawAbilityHUDSingle(int player, int ability, float cd, float active, float hx, float hy) {
  if (ability < 0) return;
  ...
  if (cd > 0) {
    ...
    float pulse = 0.7f + 0.3f * sin(millis() / 150.0f);
...
public void updateBot() {
  ...
  if (botReactionTimer <= 0) {
    ...
    if (ballDX > 0) {
      float timeToReach = (width - 20 - paddleW - ballX) / max(abs(ballDX), 0.1f);
      ...
      botTargetY = height / 2 + botMistakeOffset * 0.5f;
    }
...
public void updateBotAbility() {
  ...
  float baseDelay;
  float useChance;
  if (difficulty == 0) { baseDelay = 3.0f; useChance = 0.3f; }
  else if (difficulty == 1) { baseDelay = 1.5f; useChance = 0.6f; }
  else { baseDelay = 0.5f; useChance = 0.9f; }

  botAbilityTimer = baseDelay + random(0.5f);
  ...
  switch (p2Ability) {
    case AB_SHIELD:
      shouldUse = !p2Shield && ballDX > 0 && distToBall > paddle2H * 0.8f;
      break;
    case AB_GROW:
      shouldUse = ballDX > 0;
      break;
    case AB_SLOW:
      shouldUse = ballDX > 0 && sqrt(ballDX * ballDX + ballDY * ballDY) > ballBaseSpeed * 1.3f;
      break;
    case AB_POWER:
      shouldUse = ballDX < 0;
      break;
    case AB_CURVE:
      ...
      break;
    case AB_DASH:
      shouldUse = distToBall > paddle2H * 0.7f && ballDX > 0;
      if (shouldUse) {
        if (ballY < paddleCenter) upPressed = true; else downPressed = true;
      }
      break;
    case AB_BOOST:
      shouldUse = ballDX > 0 && distToBall > paddle2H * 0.5f;
      break;
    case AB_TELEPORT:
...
// ===================== ABILITY ACTIVATION =====================
public void activateAbility(int player) {
  ...
  switch (ability) {
    case AB_GROW:
      if (player == 1) { paddle1H = basePaddleH * 1.8f; p1ActiveTimer = duration; }
      else { paddle2H = basePaddleH * 1.8f; p2ActiveTimer = duration; }
    ...
    case AB_BOOST:
      if (player == 1) { paddle1Speed = basePaddleSpeed * 2.2f; p1ActiveTimer = duration; }
      else { paddle2Speed = basePaddleSpeed * 2.2f; p2ActiveTimer = duration; }
      break;
...
public void endAbility(int player) {
...
public void handlePaddleHit(int player) {
  ballDX *= -1;
  rallyCount++;
  ...
  float spd = ballSpeed + rallyCount * 0.12f;
  spd = min(spd, 16);
  ...
  shakeAmount = max(shakeAmount, 3 + rallyCount * 0.3f);
}

// ===================== PARTICLES =====================
public void spawnParticles2(float x2, float y2, int count, int r, int g, int b) {
...
// ===================== SCREEN FX =====================
public void flashScreen(int r, int g, int b, float alpha) {
  flashR = r;
  flashG = g;
  flashB = b;
  flashAlpha = alpha;
...
public void handleMatchEnd(boolean player1Won) {
...
public void drawGameOverScreen() {
  ...
  float pulse = 0.7f + 0.3f * sin(millis() / 300.0f);
...
// ===================== LEVEL UP =====================
public void drawLevelUpOverlay() {
  int[] acc = skinAccent[currentSkin];
  float a = min(levelUpTimer * 80, 200);
  fill(acc[0], acc[1], acc[2], a * 0.15f);
  noStroke();
  rect(0, 0, width, height);

  float bannerY = 50 + sin(levelUpAnimFrame * 0.05f) * 5;
  fill(0, 0, 0, a * 0.6f);
...
public void drawLevelUpScreen() {
  ...
  int[] acc = skinAccent[currentSkin];
  float scale = 1.0f + 0.1f * sin(millis() / 200.0f);
  textSize(80 * scale);
  fill(acc[0], acc[1], acc[2], 40);
...
public void drawStarfield() {
  noStroke();
  float t = millis() / 50.0f;
  for (int i = 0; i < 60; i++) {
    float sx2 = (noise(i * 100) * width + t * (0.3f + noise(i * 50) * 0.5f)) % width;
    float sy2 = noise(i * 200 + 500) * height;
    float sz = 1 + noise(i * 300) * 2.5f;
    fill(255, 40 + noise(i * 400) * 40);
    ellipse(sx2, sy2, sz, sz);
  }
}

public void drawNeonGrid() {
  stroke(0, 40, 60, 40);
  strokeWeight(1);
  float offset = (millis() / 40.0f) % 40;
...
public void drawRetroScanlines() {
...
public void keyPressed() {
  ...
  if (gameState == STATE_MENU) {
    ...
    if (escWasPressed) { menuHover = 1; startTransition(STATE_MENU); }
  }
...
// ===================== BOT ABILITY PICK =====================
public void botPickAbility() {
  if (difficulty == 0) {
    p2Ability = PApplet.parseInt(random(AB_COUNT));
  } else if (difficulty == 1) {
    int[] good = {AB_SHIELD, AB_GROW, AB_TELEPORT, AB_DASH, AB_BOOST};
    p2Ability = good[PApplet.parseInt(random(good.length))];
  } else {
    int[] best = {AB_SHIELD, AB_TELEPORT, AB_GROW};
    p2Ability = best[PApplet.parseInt(random(best.length))];
  }
}
