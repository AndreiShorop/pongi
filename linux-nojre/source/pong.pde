...
final float[] AB_COOLDOWN = {8, 30, 7, 6, 5, 4, 6, 8};
...
// ===================== INPUT =====================
void keyPressed() {
  boolean escWasPressed = (key == ESC);
  if (key == ESC) key = 0; // prevent fullscreen exit
  ...
  if (gameState == STATE_MENU) {
    ...
    if (escWasPressed) { menuHover = 0; startTransition(STATE_MENU); }
  }
    ...
    if (escWasPressed) { menuHover = 0; startTransition(STATE_MODE); }
  }
    ...
    if (escWasPressed) { menuHover = 1; skinsMenuTab = 0; startTransition(STATE_MENU); }
  }
    ...
    if (escWasPressed) { menuHover = 2; startTransition(STATE_MENU); }
  }
    ...
    if (escWasPressed) { abilityMenuSelection = 0; startTransition(STATE_SELECT_P1); }
  }
    ...
    if (escWasPressed) { menuHover = 0; gameState = STATE_MENU; }
  }
