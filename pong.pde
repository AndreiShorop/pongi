...
void saveProgress() {
  String[] lines = { str(playerXP), str(playerLevel), str(currentSkin), str(ballSpeedSetting) };
...
void loadProgress() {
  try {
    ...
    if (lines != null && lines.length >= 4) {
      playerXP = int(lines[0]);
      playerLevel = int(lines[1]);
      currentSkin = constrain(int(lines[2]), 0, SKIN_COUNT - 1);
      ballSpeedSetting = constrain(int(lines[3]), 0, 2);
      xpToNext = calcXPtoNext(playerLevel);
    }
  } catch (Exception e) {
  }
...
void drawXPBar(float x, float y, float w, float h) {
  ...
  fill(255);
  textSize(12);
  text("Lv." + playerLevel + "  " + playerXP + "/" + xpToNext + " XP", x + w / 2, y + h / 2 - 1);
}
...
void drawSkinsMenu() {
  ...
  for (int i = 0; i < SKIN_COUNT; i++) {
    ...
    noStroke();
    fill(skinBG[i][0], skinBG[i][1], skinBG[i][2]);
    rect(bx, by, bw, bh, 10);
    ...
    strokeWeight(1);
    for (int dy = (int)(by + 10); dy < by + bh - 10; dy += 12) {
      line(bx + bw / 2, dy, bx + bw / 2, dy + 6);
    }
    ...
    noStroke();
    fill(hovered || selected ? 255 : 150);
    textSize(16);
    text(SKIN_NAMES[i], bx + bw / 2, by + bh + 22);

    if (selected) {
      fill(skinAccent[i][0], skinAccent[i][1], skinAccent[i][2]);
      textSize(12);
      text("ACTIVE", bx + bw / 2, by + bh + 42);
    }
  }

  fill(100);
  textSize(14);
  text("LEFT/RIGHT = browse | ENTER = select | ESC = Back", width / 2, height - 20);
}
...
void drawGameOverScreen() {
  ...
  fill(acc[0], acc[1], acc[2]);
  textSize(18);
  String xpMsg = (gameMode == 1 && p1Won) ? "+100 XP" : (gameMode == 1 ? "+25 XP" : "+50 XP");
...
void drawLevelUpOverlay() {
  ...
  fill(0, 0, 0, a * 0.6);
  rect(width / 2 - 160, bannerY - 5, 320, 50, 10);
...
void keyPressed() {
  ...
  if (gameState == STATE_MENU) {
    ...
    if (key == ENTER || key == RETURN) {
      ...
      else if (menuHover == 1) { menuHover = currentSkin; startTransition(STATE_SKINS); }
    ...
    if (key == ENTER || key == RETURN) { currentSkin = menuHover; saveProgress(); }
    if (key == ESC) { key = 0; menuHover = 1; startTransition(STATE_MENU); }
  }
